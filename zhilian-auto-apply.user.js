// ==UserScript==
// @name         智联招聘自动投递助手（无AI）
// @namespace    local.zhilian.auto.apply
// @version      1.0.0
// @description  智联招聘推荐页自动投递：自动扫描、过滤、历史跳过、滚动加载、弹窗确认；不含AI、不连接第三方服务。
// @author       local
...
(() => {
  ...
  const STORAGE_KEY = 'zhilian_auto_apply_settings_v1';
  const HISTORY_KEY = 'zhilian_auto_apply_history_v1';
  const HISTORY_MAX = 3000;
  const HISTORY_TTL = 1000 * 60 * 60 * 24 * 180;
  const EMPTY_SCROLL_STOP_BATCHES = 5;
  const JOB_LINK_PATTERN = /\/job\/|\/jobdetail\/|\/jobs\/|\/position\/|\/zpurl\/|jobId=|positionId=|position_id=|job_id=|number=/i;
  const SEARCH_PAGE_PATTERN = /\/recommend|\/sou|\/job|\/jobs|\/position/i;
  const JOB_CARD_SELECTOR = [
    ...
    '[class*=joblist]', '[class*=jobList]', '[class*=positionlist]', '[class*=positionList]', '[class*=position-item]', '[class*=positionItem]',
    '[class*=recommend-card]', '[class*=recommendCard]', '[class*=recommend-job]', '[class*=recommendJob]', '[class*=zp-job]', '[class*=zpJob]',
  ...
  const APPLY_TEXT_PATTERN = /^(?:\u7acb\u5373|\u9a6c\u4e0a|\u4e00\u952e|\u5feb\u901f)?(?:\u6295\u9012|\u5e94\u8058|\u7533\u8bf7|\u53d1\u9001)(?:\u7b80\u5386|\u804c\u4f4d|\u5c97\u4f4d)?$|^(?:\u7acb\u5373)?(?:\u6c9f\u901a|\u6c9f\u901aHR|\u5f00\u59cb\u6c9f\u901a|\u5728\u7ebf\u6c9f\u901a|\u7ee7\u7eed\u6c9f\u901a)$|^(?:\u7ee7\u7eed)?\u804a(?:\u4e00)?\u804a$|^(?:\u7acb\u5373)?\u8054\u7cfb(?:TA|HR|\u6211|\u5bf9\u65b9)?$|^\u6253\u62db\u547c$/i;
  ...
  const OPEN_CHAT_API_PATTERN = /apply|delivery|deliver|sendresume|send-resume|resume.*job|job.*resume|jobapply|position.*apply|candidate|jobseeker|chat|\bim\b/i;
  const CHAT_OPEN_TEXT_PATTERN = /投递成功|申请成功|已投递|已申请|继续沟通|沟通中|发送消息|聊天|消息|chat|im/i;
  const CHAT_BLOCK_TEXT_PATTERN = /简历不完善|立即完善|选择简历|完善简历|求职状态|简历未公开|登录|验证|风控|频繁|上限|异常|请稍后/i;
  const SAFETY_TIP_PATTERN = /求职过程|收取培训费|考证费|中介费|押金|虚假招聘|立即举报|安全提示|防骗提示/i;
  ...
  const T = {
    ...
    confirmClear: '\u786e\u5b9a\u6e05\u7a7a\u667a\u8054\u62db\u8058\u81ea\u52a8\u6295\u9012\u7684\u672c\u5730\u5386\u53f2\u5417\uff1f',
    stoppedByUser: '\u5df2\u624b\u52a8\u505c\u6b62'
  };
  ...
  const patchNetwork = () => {
    if (window.__zhilianAutoApplyNetworkPatched) return;
    window.__zhilianAutoApplyNetworkPatched = true;
  ...
  const injectStyle = () => {
    if (document.getElementById('zhilian-auto-apply-style')) return;
    const style = document.createElement('style');
    style.id = 'zhilian-auto-apply-style';
    style.textContent = [
      '#zhilian-auto-apply-panel{position:fixed;right:20px;top:90px;width:360px;z-index:2147483647;background:rgba(255,255,255,.98);color:#222;border:1px solid #dcdfe6;border-radius:10px;box-shadow:0 8px 28px rgba(0,0,0,.16);font:13px/1.4 -apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif;overflow:hidden}',
      '#zhilian-auto-apply-panel *{box-sizing:border-box}',
      '#zhilian-auto-apply-panel.zp-aa-collapsed{width:190px}',
      '.zp-aa-header{display:flex;align-items:center;justify-content:space-between;padding:9px 10px;background:#0052d9;color:#fff;cursor:move;user-select:none}',
      '.zp-aa-header strong{font-size:14px;font-weight:700}',
      '.zp-aa-header button{width:24px;height:24px;border:0;border-radius:6px;background:rgba(255,255,255,.18);color:#fff;cursor:pointer}',
      '.zp-aa-body{padding:10px}',
      '.zp-aa-collapsed .zp-aa-body{display:none}',
      '.zp-aa-row{display:flex;align-items:center;gap:8px;margin-bottom:8px}',
      '.zp-aa-row label{flex:1;color:#333}',
      '.zp-aa-row input[type=number]{width:90px;border:1px solid #dcdfe6;border-radius:6px;padding:5px 6px}',
      '.zp-aa-textarea{width:100%;min-height:48px;max-height:90px;resize:vertical;border:1px solid #dcdfe6;border-radius:7px;padding:6px;margin:0 0 8px;font:12px/1.4 -apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif}',
      '.zp-aa-inline{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:4px 0 6px;color:#606266;font-size:12px}',
      '.zp-aa-link{border:0;background:transparent;color:#0052d9;cursor:pointer;padding:0;font-size:12px}',
      '.zp-aa-disabled{opacity:.55}',
      '.zp-aa-disabled input{cursor:not-allowed;background:#f5f7fa;color:#999}',
      '.zp-aa-check{justify-content:flex-start}',
      '.zp-aa-check input{margin:0}',
      '.zp-aa-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:10px 0}',
      '.zp-aa-actions button,.zp-aa-secondary button{border:0;border-radius:7px;padding:7px 8px;cursor:pointer;background:#edf3ff;color:#0052d9;font-weight:600}',
      '.zp-aa-actions button.zp-aa-primary{background:#0052d9;color:#fff}',
      '.zp-aa-actions button.zp-aa-danger{background:#fff1f0;color:#cf1322}',
      '.zp-aa-actions button:disabled,.zp-aa-secondary button:disabled{background:#e5e7eb;color:#9ca3af;box-shadow:none;cursor:not-allowed;opacity:.8}',
      '.zp-aa-secondary{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px}',
      '.zp-aa-status{display:flex;justify-content:space-between;gap:8px;border-top:1px solid #ebeef5;padding-top:8px;margin-top:8px;color:#606266;font-size:12px}',
      '.zp-aa-log{height:150px;overflow:auto;border:1px solid #ebeef5;border-radius:7px;background:#fafafa;padding:6px;font-size:12px;color:#303133}',
      '.zp-aa-log div{margin-bottom:4px;word-break:break-word}',
      '.zp-aa-log .ok{color:#067d17}',
      '.zp-aa-log .warn{color:#ad6800}',
      '.zp-aa-log .error{color:#cf1322}',
      '.zp-aa-log .muted{color:#909399}'
    ].join('\n');
    document.head.appendChild(style);
  };

  const createPanel = () => {
    if (document.getElementById('zhilian-auto-apply-panel')) return;
    injectStyle();
    const panel = document.createElement('div');
    panel.id = 'zhilian-auto-apply-panel';
    ...
    if (settings.panelCollapsed) panel.classList.add('zp-aa-collapsed');
    panel.innerHTML = [
      '<div class="zp-aa-header" id="zp-aa-drag"><strong>', T.panelTitle, '</strong><button type="button" id="zp-aa-collapse" title="', T.collapse, '">', settings.panelCollapsed ? '+' : '-', '</button></div>',
      '<div class="zp-aa-body">',
      '<div class="zp-aa-row"><label for="zp-aa-max">', T.maxApply, '</label><input id="zp-aa-max" type="number" min="1" max="999" step="1"></div>',
      '<div class="zp-aa-row"><label for="zp-aa-interval">', T.interval, '</label><input id="zp-aa-interval" type="number" min="1" max="60" step="1"></div>',
      '<div class="zp-aa-row" id="zp-aa-active-row"><label for="zp-aa-active-days">', T.activeHrDays, '</label><input id="zp-aa-active-days" type="number" min="0" max="365" step="1"></div>',
      '<label class="zp-aa-row zp-aa-check"><input id="zp-aa-auto" type="checkbox"><span>', T.autoLoadMore, '</span></label>',
      '<label class="zp-aa-row zp-aa-check"><input id="zp-aa-history" type="checkbox"><span>', T.skipHistory, '</span></label>',
      '<label class="zp-aa-row zp-aa-check"><input id="zp-aa-only-foreign" type="checkbox"><span>', T.onlyForeign, '</span></label>',
      '<label class="zp-aa-row zp-aa-check"><input id="zp-aa-block-outsource" type="checkbox"><span>', T.blockOutsource, '</span></label>',
      '<label class="zp-aa-row zp-aa-check"><input id="zp-aa-only-active" type="checkbox"><span>', T.onlyActiveHr, '</span></label>',
      '<div class="zp-aa-inline"><span>', T.foreignKeywords, '</span><button type="button" class="zp-aa-link" id="zp-aa-reset-foreign">', T.resetForeign, '</button></div>',
      '<textarea class="zp-aa-textarea" id="zp-aa-foreign-keywords"></textarea>',
      '<div class="zp-aa-inline"><span>', T.outsourceKeywords, '</span><button type="button" class="zp-aa-link" id="zp-aa-reset-outsource">', T.resetOutsource, '</button></div>',
      '<textarea class="zp-aa-textarea" id="zp-aa-outsource-keywords"></textarea>',
      '<div class="zp-aa-actions"><button type="button" class="zp-aa-primary" id="zp-aa-start">', T.start, '</button><button type="button" class="zp-aa-danger" id="zp-aa-stop">', T.stop, '</button></div>',
      '<div class="zp-aa-secondary"><button type="button" id="zp-aa-scan">', T.scan, '</button><button type="button" id="zp-aa-clear">', T.clearHistory, '</button></div>',
      '<div class="zp-aa-status"><span id="zp-aa-run-state">', T.ready, '</span><span id="zp-aa-stats"></span></div>',
      '<div class="zp-aa-log" id="zp-aa-log"></div>',
      '</div>'
    ].join('');
  ...
  const bindPanel = (panel) => {
    const maxInput = byId('zp-aa-max');
    const intervalInput = byId('zp-aa-interval');
    const activeDaysInput = byId('zp-aa-active-days');
    const autoInput = byId('zp-aa-auto');
    const historyInput = byId('zp-aa-history');
    const onlyForeignInput = byId('zp-aa-only-foreign');
    const blockOutsourceInput = byId('zp-aa-block-outsource');
    const onlyActiveInput = byId('zp-aa-only-active');
    const foreignKeywordsInput = byId('zp-aa-foreign-keywords');
    const outsourceKeywordsInput = byId('zp-aa-outsource-keywords');
    ...
    byId('zp-aa-reset-foreign').addEventListener('click', () => {
    ...
    byId('zp-aa-reset-outsource').addEventListener('click', () => {
    ...
    byId('zp-aa-start').addEventListener('click', () => {
      const startButton = byId('zp-aa-start');
    ...
    byId('zp-aa-stop').addEventListener('click', () => stopAutoApply(T.stoppedByUser));
    byId('zp-aa-scan').addEventListener('click', () => {
      captureJobsFromDom();
      log(T.scanFound + state.capturedJobs.size, 'muted');
      updatePanel();
    });
    byId('zp-aa-clear').addEventListener('click', () => {
      if (!window.confirm(T.confirmClear)) return;
      history.clear();
      saveHistory();
      log(T.historyCleared, 'warn');
    });
    byId('zp-aa-collapse').addEventListener('click', (event) => {
      event.stopPropagation();
      settings.panelCollapsed = !settings.panelCollapsed;
      panel.classList.toggle('zp-aa-collapsed', settings.panelCollapsed);
      event.currentTarget.textContent = settings.panelCollapsed ? '+' : '-';
      saveSettings();
    });

    bindDrag(panel, byId('zp-aa-drag'));
  };

  const updateActiveDaysInputState = () => {
    const input = byId('zp-aa-active-days');
    const row = byId('zp-aa-active-row');
    const onlyActive = byId('zp-aa-only-active');
    if (!input || !row || !onlyActive) return;
    const disabled = !onlyActive.checked;
    input.disabled = disabled;
    row.classList.toggle('zp-aa-disabled', disabled);
  };
  ...
  const updatePanel = () => {
    const runState = byId('zp-aa-run-state');
    const stats = byId('zp-aa-stats');
    const start = byId('zp-aa-start');
    const stop = byId('zp-aa-stop');
